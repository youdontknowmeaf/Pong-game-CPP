...
//----------------------------------[ Konfiguracja ]--------------------------------------------------------------------------//
	...
	int MaxFps = 45; // Maksymalna liczba FPS
					 // Max FPS limit
	int speed_ball = 7; // Predkosc pileczki (ostrożnie)
						// Ball speed (careful)
    int ball_radius     // Promień koła (piłki)
...
class Ball{
    ...
    void update() {
        ...
        if(y + ball_radius >= GetScreenHeight() || y - radius <= 0)
        {
            speed_y *= -1;
        }
        if(x + ball_radius >= GetScreenWidth() || x - radius <= 0)
        {
            speed_x *= -1;
        }
    }
